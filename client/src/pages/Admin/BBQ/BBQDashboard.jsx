...
const BBQDashboard = () => {
  ...
  const { loading, data, error, fetchBBQData, reset } = useGetAPIStore();
  ...
  const fetchUsers = async (currentPage = 1) => {
    fetchBBQData(`${API_URL_BBQ}/showProject?page=${currentPage}&limit=5`);
  };
  ...
  const actions = [
    {
      label: "View",
      onClick: (item) => navigate("/ViewBBQ", { state: { project: item } }),
      className: "view-button",
      title: "View Details",
    },
    {
      label: "Edit",
      onClick: (item) => alert(`View clicked for ${item.name}`),
      className: "view-button",
      title: "View Details",
    },
    {
